...
though. It’s relatively new[^03.2] so there is not a huge set of
...
can download it from the jamovi homepage, which is: [https://www.jamovi.org/](https://www.jamovi.org/)
...
you’ll probably have a newer version.[^03.3]
...
![*Figure 3.1: jamovi starts up!*](images/Figure2.PNG)
...
levels ([Figure 3.2](#Fig3.2)).

<a name="Fig3.2"></a>

![*Figure 3.2: measurement levels*](images/Figure3.PNG)
...
len, the mean of A and B, and the z-score of the variable dose. [Figure 3.3](#Fig3.3)
below shows the jamovi screen for the new variable computed as the
z-score of dose (from the ‘Tooth Growth’ example data set).

<a name="Fig3.3"></a>

![*Figure 3.3: A newly computed variable, the z-score of ‘dose’*](images/Figure4.PNG)
...
![*Figure 3.4: The booksales.csv data file. On the left I’ve opened the
file using a spreadsheet program (OpenOffice), which shows that the file
is basically a table. On the right the same file is open in a standard
text editor (the TextEdit program on a Mac), which shows how the file is
formatted. The entries in the table are wrapped in quote marks and
separated by commas*](images/Figure5.PNG)
...
in [Figure 3.4](#Fig3.4), which shows a file called booksales.csv that I’ve
...
like on a Mac is shown in [Figure 3.5](#Fig3.5). I’m assuming that you’re
...
![*Figure 3.5: A dialog box on a Mac asking you to select the csv file
jamovi should try to import. Mac users will recognise this immediately,
it’s the usual way in which a Mac asks you to find a file. Windows users
won’t see this, instead they’ll see the usual explorer window that
Windows always gives you when it wants you to select a
...
-   missing values. Often you’ll get given data with missing values. For
    ...
    99[^03.4], for both numeric and text data, so you should
...
want, and then install them, as in [Figure 3.6](#Fig3.6). It’s that easy. The newly
installed modules can then be accessed from the “Analyses” button bar.
Try it...useful add-on modules to install include ”scatr” (added under
“Descriptives”) and $R_j$.

<a name="Fig3.6"></a>

![*Figure 3.6: Installing add-on modules in jamovi*](images/Figure7.PNG)
...
-   [Analyses]. We very briefly oriented to the part of jamovi where
    analyses are done and results appear, but then deferred this until
    later in the book.
-   [The spreadsheet]. We spent more time looking at the spreadsheet part of
...
-   [Importing unusual data files]. Then we figured out how to open other data files, from
    different file types.
-   [Changing data from one level to another]. And saw that sometimes we need to coerce data from one
    type to another.
-   [Installing add-on modules into jamovi]. Installing add-on modules from the jamovi community
    really extends jamovi capabilities.
-   [Quitting jamovi]. Finally, we looked at good practice in terms of saving
...
[^03.1]: Source: Dismal Light (1968).

[^03.2]: As of the first writing this in August 2018. Later versions of this book
will use later versions of jamovi.

[^03.3]: Although jamovi is updated frequently it doesn’t usually make much
...
[^03.4]: You can change the default value for missing values in jamovi from
...
[^03.5]: I know this is a bit of a fudge, but it does work and hopefully
