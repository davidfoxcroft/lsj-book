...
ggplot(afl.margins, aes(x=margin)) +
  geom_histogram(breaks=seq(0,120,10), col = "black", fill = c("white", rep("blue",5), rep("white",6)), alpha = 0.6) +
  theme_classic() +
...
skewplots <- lapply(1:3, function(i) {
  ggplot(X[[i]], aes(x=value)) +
    geom_histogram(binwidth=0.04, col="white", fill="blue", alpha=0.6) +
    theme_classic() +
    ggtitle(X_labels[[i]]) +
    theme(axis.title.x = element_blank(),
        axis.line.x = element_blank(),
...
kurtplots <- lapply(1:3, function(i) {
  ggplot(X[[i]], aes(x=value)) +
    geom_histogram(binwidth=0.5, col="white", fill="blue", alpha=0.6) +
