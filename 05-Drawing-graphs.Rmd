...
data from the [Descriptive statistics] chapter and what you want to do is get an overall
...
**bins** and then count the number of observations that fall within each
...
we showed earlier in the [Descriptive statistics] chapter, Figure \@ref(fig:fig4-2). With these earlier
...
```{r fig5-2, echo=FALSE, fig.cap="jamovi screen showing the histogram check box", out.width = "90%"}
...
chart is a variation of a histogram that uses **kernel smoothing** to
...
```{r fig5-3, echo=FALSE, fig.cap="A density plot of the afl.margins variable plotted in jamovi", out.width = "90%"}
...
```{r fig5-4, echo=FALSE, fig.cap="A box plot of the afl.margins variable plotted in jamovi", out.width = "90%"}
...
and is commonly referred to as an **outlier**. For our AFL margins data
...
```{r fig5-5, echo=FALSE, fig.cap="A violin plot of the afl.margins variable plotted in jamovi, also showing a box plot and data points", out.width = "90%"}
...
stored in the **aflmarginbyyear.csv** file. So let’s load it into jamovi
...
```{r fig5-6, echo=FALSE, fig.cap="jamovi screen shot showing the ‘Split by’ window", out.width = "90%"}
...
```{r fig5-7, echo=FALSE, fig.cap="Multiple boxplots plotted in jamovi, for the margin by year variables in the aflsmall2 data set", out.width = "90%"}
...
window, and then type ‘margin \> 300’ into the filter field, as in
Figure \@ref(fig:fig5-9).

```{r fig5-8, echo=FALSE, fig.cap="A boxplot showing two very suspicious outliers!", out.width = "90%"}
...
```{r fig5-9, echo=FALSE, fig.cap="The jamovi filter screen", out.width = "90%"}
knitr::include_graphics("images/Figure30.PNG")
```

```{r fig5-10, echo=FALSE, fig.cap="Frequency table for ID showing the ID numbers for the two suspicious outliers: 14 and 134", out.width = "90%"}
...
then keep it. In any case, I’ll return to the topic again in the section on [Model checking] in the chapter on [Correlation and linear regression].
...
afl.finalists $==$ ‘Fremantle’ or afl.finalists $==$ ‘Richmond’  [^05.2]
...
```{r fig5-11, echo=FALSE, fig.cap="Filtering to include just four AFL teams, and drawing a bar plot in jamovi", out.width = "90%"}
...
-   *Common plots*. Much of the chapter was focused on standard graphs
    that statisticians like to produce: [Histograms],
    [Boxplots] and [Bar graphs]
-   [Saving image files using jamovi]. Importantly, we also covered how to export
    your pictures.
...
target="_blank">https://ggplot2.tidyverse.org/</a>), which is loosely based on “The
grammar of graphics” [@Wilkinson2006]. It’s not for novices. You need to have a pretty good grasp of R before
...
[^05.1]: The origin of this quote is Tufte’s lovely book *The Visual
Display of Quantitative Information.*

[^05.2]: jamovi uses the symbol “==” here to mean “matches”.
