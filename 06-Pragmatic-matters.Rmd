...
```{r tab6-1}
huxtabs[[6]][[1]] %>% set_caption("Frequency table for the speaker variable") 
```
...
```{r fig6-1, fig.cap="Contingency table for the speaker and utterances variables", out.width = "90%"}
knitr::include_graphics("images/Figure33.PNG")
```

Don’t worry about the “$\chi^2$ Tests” table that is produced. We
are going to cover this later on in the chapter on [Categorical data analysis]. When interpreting
...
```{r fig6-2, fig.cap="Contingency table for the speaker and utterances variables, with row percentages", out.width = "90%"}
knitr::include_graphics("images/Figure34.PNG")
```

```{r fig6-3, fig.cap="Contingency table for the speaker and utterances variables, with column percentages", out.width = "90%"}
...
humans[^06.2] and it’s definitely not true of jamovi. jamovi is
...
it to calculate $2 + 2$[^06.3], it always gives the same answer,
...
jamovi to make a “true or false” judgement.[^06.4] Okay, let’s
...
$2 + 2 = 5$, regardless of whether you use the correct **equality
...
whether you use the correct **equality operator**, $==$, or the equals
sign “$=$”.
...
For example, the **less than** operator < checks to see if the number
...
are worth knowing about, listed in Table \@ref(tab:tab6-3). These are the **not**
operator !, the **and** operator and, and the **or** operator or. Like
...
```{r tab6-2}
huxtabs[[6]][[2]] %>% set_caption("Some logical operators") 
```

```{r tab6-3}
huxtabs[[6]][[3]] %>% set_caption("Some more logical operators") 
```


$$(2+2 == 4) \text{ or } (2+2 == 5)$$

On the other hand, if I ask you to assess the claim that “both $2 + 2
= 4$ and $2 + 2 = 5$” you’d say that it’s false. Since this is an
...
allows you to use the \> and \> operators to determine which of two text
...
```{r tab6-4}
huxtabs[[6]][[4]] %>% set_caption("Text characters in the order that jamovi processes them") %>% 
...
```{r fig6-4, fig.cap="Creating a new computed variable in jamovi", out.width = "90%"}
...
of the centred data (using the ‘ABS’ function).[^06.6] In
...
```{r fig6-5, fig.cap="Using the $f_x$ button to select functions and variables", out.width = "90%"}
knitr::include_graphics("images/Figure6_6.PNG")
```
...
IF(Age \>= 0 and Age \<= 20, 1, IF(Age \>= 21 and Age \<= 40, 2, IF(Age
\>= 41 and Age \<= 60, 3 )))
...
```{r fig6-6, fig.cap="Collapsing a variable into a smaller number of discrete levels using the jamovi ‘IF’ function", out.width = "90%"}
...
the categorised data instead of the original data.[^06.8] I
...
```{r fig6-7, fig.cap="Creating a new variable transformation using the jamovi ‘Transform’ command", out.width = "90%"}
knitr::include_graphics("images/Figure38.PNG")
```

```{r fig6-8, fig.cap="Specifying a transformation in jamovi, to be saved as the imaginatively named ‘Transform 1’", out.width = "90%"}
knitr::include_graphics("images/Figure39.PNG")
```

```{r fig6-9, fig.cap="jamovi transformation into three age categories, using the ‘Add condition’ button", out.width = "90%"}
knitr::include_graphics("images/Figure41.PNG")
```

```{r tab6-5}
huxtabs[[6]][[5]] %>% set_caption("Some mathematical operators") %>%  
  set_contents(1, 1, "")
```

## A few more mathematical functions and operations

In Section **6.3** I discussed the ideas behind variable transformations
...
functions I want to talk about here, or later.[^06.9] Obviously
...
as **Euler’s number**, is one of those annoying “irrational” numbers
...
Raising e to the power $x$ is called the **exponential** of $x$,
...
the two variables speaker and utterance (see Section **6.1** if you’ve
...
```{r fig6-10, echo=FALSE, fig.cap="Creating a subset of the nightgarden data using the jamovi ‘Filters’ option", out.width = "90%"}
...
with the cases where speaker is <u>not</u> ‘makka-pakka’ greyed-out
(i.e., filtered out) and, conversely, the cases where speaker <u>is</u>
...
[^06.1]: The quote comes from Home is the Hangman, published in 1975.

[^06.2]: I offer up my teenage attempts to be “cool” as evidence that some
things just can’t be done.

[^06.3]: You can do this in the Compute new variable screen, though just
calculating 2 + 2 for every cell of a new variable is not very useful!

[^06.4]: Note that this is a very different operator to the equals operator
...
And why I’m still not cool.

[^06.5]: Now, here’s a quirk in jamovi. When you have simple logical
...
[^06.6]: The absolute value of a number is its distance from zero,
regardless of whether it’s sign is negative or positive.

[^06.7]: The reason we have to use the ‘IF’ command and keep zero as zero
...
[^06.8]: If you’ve read further into the book, and are re-reading this
...
normality assumption) and you can lose a lot of statistical power.

[^06.9]: We’ll leave the box-cox function until later on
