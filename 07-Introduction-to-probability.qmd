...
ggplot(data=pantsdat, aes(x=eventNames, y=probabilities)) +
  geom_bar(stat='identity', col="black", fill="blue", alpha=0.6) +
  theme_classic() +
...
ggplot(data=binom1, aes(x = factor(heads), y = pmf)) +
  geom_col(col="blue", fill="blue", alpha=0.6, width=.1) +
...
#| fig-align: left
#| out.width: 80%
#| fig-cap: Two binomial distributions, involving a scenario in which I'm flipping a fair coin, so the underlying success probability is $\theta = \frac{1}{2}$. In panel (a), we assume I'm flipping the coin $N = 20$ times. In panel (b) we assume that the coin is flipped $N = 100$ times

binom2 <- data.frame(heads = 0:20, pmf = dbinom(x = 0:20, size = 20, prob = 1/2)+0.0001) 

ggplot(data=binom2, aes(x = factor(heads), y = pmf)) +
  geom_col(col="blue", fill="blue", alpha=0.6, width=.1) +
...
ggplot(data=binom3, aes(x = factor(heads), y = pmf)) +
  geom_col(col="blue", fill="blue", alpha=0.6, width=.1) +
  labs(x = "\nNumber of skulls observed",
       y = "Probability\n") +
...
ggplot(data = data.frame(x = seq(-3, 3, 1)), aes(x)) +
  stat_function(fun = dnorm, n = 1000, args = list(mean = 0, sd = 1), col="blue", size=1) + 
...
[^07-introduction-to-probability-4]: The notation that we sometimes use to say that a variable $X$ is normally distributed is as follows: $$X \sim Normal(\mu,\sigma)$$ Of course, that's just notation. It doesn't tell us anything interesting about the normal distribution itself. As was the case with the binomial distribution, I have included the formula for the normal distribution in this book, because I think it's important enough that everyone who learns statistics should at least look at it, but since this is an introductory text I don't want to focus on it, so I've tucked it away in this footnote.
...
ggplot(data = data.frame(x = seq(0, 10, 1)), aes(x)) +
  stat_function(fun = dnorm, n = 1000, args = list(mean = 4, sd = 1), col="blue", size=1) + 
  stat_function(fun = dnorm, n = 1000, args = list(mean = 7, sd = 1), col="blue", linetype=2, size=1) + 
...
ggplot(data = data.frame(x = seq(0, 10, 1)), aes(x)) +
  stat_function(fun = dnorm, n = 1000, args = list(mean = 5, sd = 1), col="blue", size=1) + 
  stat_function(fun = dnorm, n = 1000, args = list(mean = 5, sd = 2), col="blue", linetype=2, size=1) + 
  scale_x_continuous(breaks = seq(0, 10, 2)) +
  ylab("Probability Density\n") +
  xlab("\nObserved Value") +
  theme_classic()
...
#| fig-align: left
#| out.width: 80%
#| fig-cap: A $t$ distribution with 3 degrees of freedom (solid line). It looks similar to a normal distribution, but it's not quite the same. For comparison purposes I've plotted a standard normal distribution as the dashed line

ggplot(data = data.frame(x = seq(-4, 4, 1)), aes(x)) +
  stat_function(fun = dt, n = 1000, args = list(df = 3), col="blue", size=1) + 
  stat_function(fun = dnorm, n = 1000, args = list(mean = 0, sd = 1), col="grey", linetype=5, size=1) + 
...
ggplot(data = data.frame(x = seq(0, 10, 1)), aes(x)) +
  stat_function(fun = dchisq, n = 1000, args = list(df = 3), col="blue", size=1) + 
  scale_x_continuous(breaks = seq(0, 10, 2)) +
  ylab("Probability Density\n") +
  xlab("\nObserved Value") +
  theme_classic()
...
#| fig-align: left
#| out.width: 80%
#| fig-cap: An $F$ distribution with 3 and 5 degrees of freedom. Qualitatively speaking, it looks pretty similar to a chi-square distribution, but they're not quite the same in general

ggplot(data = data.frame(x = seq(0, 10, 1)), aes(x)) +
  stat_function(fun = df, n = 1000, args = list(df1 = 3, df2 = 5), col="blue", size=1) + 
  scale_x_continuous(breaks = seq(0, 10, 2)) +
  ylab("Probability Density\n") +
  xlab("\nObserved Value") +
  theme_classic()
