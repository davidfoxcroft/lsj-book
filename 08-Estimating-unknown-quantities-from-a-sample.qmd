...
plota <- ggplot(data = data.frame(x = seq(60, 140, 1)), aes(x)) +
  stat_function(fun = dnorm, n = 1000, args = list(mean = 100, sd = 15), col=blueshade, size=1) +
...
ggplot(data = sdm_data_means, aes(x = sample_means )) +
  geom_histogram(aes(y =..density..), binwidth=4, col=blueshade, fill=blueshade, alpha=0.7) +
...
sampling_plots <- lapply(sampling_plots_df$n, function(i) {
  ...
  ggplot(data=sdm_data_mean, aes(x=sample_mean)) +
  geom_histogram(aes(y =..density..), binwidth=0.0339, col=blueshade, fill=blueshade, alpha=0.6) +
