...
```{r fig11-1, chunk.class="enlarge.image", fig.cap="The theoretical distribution (solid line) from which the psychology student grades (bars) are supposed to have been generated", out.width = "70%"}
...
```{r fig11-6, chunk.class="enlarge.image",  fig.cap="jamovi does the one-sample t-test", out.width = "90%"}
...
```{r fig11-7, chunk.class="enlarge.image",  fig.cap="Histograms showing the distribution of grades for students in Anastasia's (panel a) and in Bernadette's (panel b) classes. Visually, these suggest that students in Anastasia's class may be getting slightly better grades on average, though they also seem a bit more variable", out.width = "90%"}
knitr::include_graphics("images/Figure87.PNG")
```

```{r fig11-8, chunk.class="enlarge.image", fig.cap="The plots show the mean grade for students in Anastasia's and Bernadette's tutorials. Error bars depict $95\\%$ confidence intervals around the mean. Visually, it does look like there's a real difference between the groups, though it's hard to say for sure", out.width = "90%"}
...
Statistical notation [^11-comparing-two-means-12]
...
Statistical notation [^11-comparing-two-means-13]
...
```{r fig11-10, chunk.class="enlarge.image", fig.cap="Independent t-test in jamovi, with options checked for useful results", out.width = "90%"}
...
Statistical notation [^11-comparing-two-means-15]

[^11-comparing-two-means-15]: But you can still estimate the standard error of the difference between sample means, it just ends up looking different $$SE(\bar{X}_1-\bar{X}_2)=\sqrt{\frac{\hat{\sigma}_1^2}{N_1}+\frac{\hat{\sigma}_2^2}{N_2}}$$ The reason why it's calculated this way is beyond the scope of\
...
```{r fig11-12, chunk.class="enlarge.image", fig.cap="Results showing the Welch test alongside the default Student's t-test in jamovi", out.width = "90%"}
...
```{r fig11-13, chunk.class="enlarge.image", fig.cap="Descriptives for the two grade test variables in the chico data set", out.width = "90%"}
...
```{r fig11-15, chunk.class="enlarge.image", fig.cap="Results showing a one sample t-test on paired difference scores", out.width = "90%"}
...
```{r fig11-16, chunk.class="enlarge.image", fig.cap="Results showing a paired sample t-test. Compare with Figure \\@ref(fig:fig11-15)", out.width = "90%"}
...
```{r fig11-17, chunk.class="enlarge.image", fig.cap="jamovi results showing a 'One Sample T-Test' where the actual hypothesis is one sided, i.e. that the true mean is greater than $67.5\\%$", out.width = "90%"}
...
```{r fig11-18, chunk.class="enlarge.image", fig.cap="jamovi results showing an 'Independent Samples t-Test' where the actual hypothesis is one sided, i.e. that Anastasia's students had higher grades than Bernadette's", out.width = "90%"}
...
```{r fig11-19, chunk.class="enlarge.image", fig.cap="jamovi results showing a 'Paired Samples T-Test' where the actual hypothesis is one sided, i.e. that grade test2 ('Measure 1') $>$ grade test1 ('Measure 2')", out.width = "90%"}
...
```{r fig11-20, chunk.class="enlarge.image", fig.cap="Histogram (panel a) and normal QQ plot (panel b) of normal.data, a normally distributed sample with $100$ observations. The Shapiro-Wilk statistic associated with these data is $W = .99$, indicating that no significant departures from normality were detected ($p = .73)$", out.width = "90%"}
...
Statistical notation [^11-comparing-two-means-22]
...
```{r fig11-24, chunk.class="enlarge.image", fig.cap="jamovi screen showing results for one sample and paired sample Wilcoxon nonparametric tests", out.width = "90%"}
