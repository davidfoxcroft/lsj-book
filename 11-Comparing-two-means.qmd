...
though to be honest this notation doesn't add much to our understanding of the problem, it's just a compact way of writing down what we're trying to learn from the data. The null hypotheses $H_0$ and the alternative hypothesis $H_1$ for our test are both illustrated in @fig-fig11-2. 
...
#| fig-alt: Two side-by-side bell curve graphs comparing the null hypothesis and alternative hypothesis. The left graph shows the null hypothesis with $\mu = \mu_0$ and $\sigma = \sigma_0$. The right graph shows the alternative hypothesis with $\mu \neq \mu_0$ and $\sigma = \sigma_0$. The x-axis is labeled "Value of X".
...
p1 <- ggplot(data = data.frame(x = seq(-4, 4, 1)), aes(x)) +
  ...
  geom_text(x=-2.2, y=.25, label=expression(~sigma~" = "~sigma[0]), size=5, check_overlap = TRUE) +
  geom_text(x= 0, y=.42, label=expression(~mu~" = "~mu[0]), size=5, check_overlap = TRUE) +
...
p2 <- ggplot(data = data.frame(x = seq(-4, 4, 1)), aes(x)) +
  ...
  geom_segment(x = 0, y = 0, xend = 0, yend = dnorm(0)) +
  geom_text(x=-3, y=.25, label=expression(~sigma~" = " ~sigma[0]), size=5, check_overlap = TRUE) +
  geom_text(x= 0, y=.42, label=expression(~mu~" \U2260 "~mu[0]), size=5, check_overlap = TRUE) +
  scale_x_continuous(breaks = seq(-4, 4, 2)) +
  ylim(0,0.5) +
...
In addition to providing us with these hypotheses, the scenario outlined above provides us with a fair amount of background knowledge that might be useful. Specifically, there are two special pieces of information that we can add:
...
#| fig-alt: Two graphs compare the null and alternative hypotheses. The left graph shows a normal distribution curve with $\mu$ = $\mu$₀, while the right graph shows a normal distribution curve with $\mu$ ≠ $\mu$₀. Both graphs have σ = ?? and "Value of X" on the x-axis.

p1 <- ggplot(data = data.frame(x = seq(-4, 4, 1)), aes(x)) +
  stat_function(fun = dnorm, n = 1000, args = list(mean = 0, sd = 1), col="black", linewidth=1) +
  geom_segment(x = 0, y = 0.25, xend = -1, yend = 0.25) +
  geom_segment(x = 0, y = 0, xend = 0, yend = dnorm(0)) +
  geom_text(x=-2.2, y=.25, label=expression(~sigma~" = ??"), size=5, check_overlap = TRUE) +
  geom_text(x= 0, y=.42, label=expression(~mu~" = "~mu[0]), size=5, check_overlap = TRUE) +
  scale_x_continuous(breaks = seq(-4, 4, 2)) +
  ylim(0,0.5) +
  ggtitle("null hypothesis") +
...
p2 <- ggplot(data = data.frame(x = seq(-4, 4, 1)), aes(x)) +
  ...
  geom_segment(x = -.75, y = 0.25, xend = -1.75, yend = 0.25) +
  geom_segment(x = 0, y = 0, xend = 0, yend = dnorm(0)) +
  geom_text(x=-3, y=.25, label=expression(~sigma~" = ??"), size=5, check_overlap = TRUE) +
  geom_text(x= 0, y=.42, label=expression(~mu~" \U2260 "~mu[0]), size=5, check_overlap = TRUE) +
  scale_x_continuous(breaks = seq(-4, 4, 2)) +
  ylim(0,0.5) +
  ggtitle("alternative hypothesis") +
...
With that much jargon crammed into half a line, you know it must be really smart.[^11-comparing-two-means-8]
...
- Normality. We're still assuming that the population distribution is normal,[^11-comparing-two-means-9] and as noted earlier, there are standard tools that you can use to check to see if this assumption is met ([Checking the normality of a sample]), and other tests you can do in its place if this assumption is violated ([Testing non-normal data]).
...
```{r out.width="80%"}
#| label: fig-fig11-7
#| fig-cap: Box and violin plots from jamovi showing the distribution of grades for students in the classes of Anastasia and Bernadette. Visually, these suggest that students in the class of Anastasia may be getting slightly better grades on average, though they also seem a bit more variable
...
#| fig-alt: Two graphs depict hypothesis testing. The left graph shows a single normal distribution curve labeled "null hypothesis" and "$\mu$". The right graph shows two overlapping normal distribution curves labeled "alternative hypothesis" with "$\mu$1" and "$\mu$2". Both graphs have "Value of X" on the x-axis.
...
p1 <- ggplot(data = data.frame(x = seq(30, 110, 10)), aes(x)) +
  ...
  geom_text(x=70, y=.047, label=expression(~mu), size=5, check_overlap = TRUE) +
  ylim(0,0.05) +
  ggtitle("null hypothesis") +
  xlab("\nValue of X") +
  theme(
...
p2 <- ggplot(data = data.frame(x = seq(30, 110, 10)), aes(x)) +
  ...
  scale_x_continuous(breaks = seq(40, 100, 20)) +
  geom_text(x=60, y=.047, label=expression(~mu[1]), size=5, check_overlap = TRUE) +
  geom_text(x=75, y=.047, label=expression(~mu[2]), size=5, check_overlap = TRUE) +
  ylim(0,0.05) +
  ggtitle("alternative hypothesis") +
  xlab("\nValue of X") +
  theme(
...
Here's the important thing. Because it really doesn't matter what jamovi shows you, I usually try to report the $t$-statistic in such a way that the numbers match up with the text. Suppose that what I want to write in my report is: *Anastasia's class had [higher]{.underline} grades than Bernadette's class*. The phrasing here implies that Anastasia's group comes first, so it makes sense to report the $t$-statistic as if Anastasia's class corresponded to group 1. If so, I would write *Anastasia's class had [higher]{.underline} grades than Bernadette's class* $(t(31) = 2.1, p = .04)$.

(I wouldn't actually underline the word "higher" in real life, I'm just doing it to emphasise the point that "higher" corresponds to positive $t$-values). On the other hand, suppose the phrasing I wanted to use has Bernadette's class listed first. If so, it makes more sense to treat her class as group 1, and if so, the write up looks like this: *Bernadette's class had* [lower]{.underline} grades than Anastasia's class $(t(31) = -2.1, p = .04)$.
...
#| fig-alt: Two bell curves depicted side by side. The graph on the left represents the null hypothesis with a single peak labeled "$\mu$." The graph on the right represents the alternative hypothesis with two overlapping peaks labeled "$\mu$1" and "$\mu$2," showing different means.

p1 <- ggplot(data = data.frame(x = seq(30, 110, 10)), aes(x)) +
  stat_function(fun = dnorm, n = 1000, args = list(mean = 70, sd = 6.5), col="black", linewidth=1) +
  scale_x_continuous(breaks = seq(40, 100, 20)) +
  geom_text(x=70, y=.066, label=expression(~mu), size=5, check_overlap = TRUE) +
  ylim(0,0.07) +
  ggtitle("null hypothesis") +
  xlab("\nValue of X") +
  theme(
...
p2 <- ggplot(data = data.frame(x = seq(30, 110, 10)), aes(x)) +
  ...
  scale_x_continuous(breaks = seq(40, 100, 20)) +
  geom_text(x=60, y=.066, label=expression(~mu[1]), size=5, check_overlap = TRUE) +
  geom_text(x=75, y=.055, label=expression(~mu[2]), size=5, check_overlap = TRUE) +
  ylim(0,0.07) +
  ggtitle("alternative hypothesis") +
  xlab("\nValue of X") +
  theme(
...
The main difference is that the standard error calculations are different. If the two populations have different standard deviations, then it's complete nonsense to try to calculate a pooled standard deviation estimate, because you're averaging apples and oranges.[^11-comparing-two-means-14]
...
The assumptions of the Welch test are very similar to those made by the Student $t$-test (see [Assumptions of the Student $t$-test], except that the Welch test does not assume homogeneity of variance. This leaves only the assumption of normality and the assumption of independence. The specifics of these assumptions are the same for the Welch test as for the Student test.
...
::: {#fig-fig11-13 layout-ncol=2 layout-valign="bottom" width=90%}

![](images/fig11-13a.png){#fig-fig11-13a fig-align="left" fig-alt="A graph comparing two variables, grade_test1 and grade_test2, shows mean and median values with their respective error bars representing 95% confidence intervals. The mean values are represented by circles, and the median values by squares." }

![](images/fig11-13b.png){#fig-fig11-13b fig-align="left" fig-alt="A scatter plot showing the relationship between scores on two tests, grade_test1 (x-axis) and grade_test2 (y-axis). Data points are clustered around a diagonal line, indicating a positive correlation between the two test scores." }
...
![Histogram from jamovi showing the improvement made by each student in Dr Chico's class. Notice that almost the entire distribution is above zero -- the vast majority of students did improve their performance from the first test to the second one](images/fig11-14.png){#fig-fig11-14 fig-align="left" fig-alt="A histogram with the x-axis labeled 'improvement' and the y-axis labeled 'density'. The bars represent the frequency of improvements, with the highest peak at approximately 1.5 on the x-axis and frequencies ranging from 0 to 4. The bars are light blue." width=60%}
...
What about the paired samples $t$-test? Suppose we wanted to test the hypothesis that grades go up from test 1 to test 2 in Dr Zeppo's class, and are not prepared to consider the idea that the grades go down. In jamovi you would do this by specifying, under the 'Hypotheses' option, that grade_test2 ('Measure 1' in jamovi, because we copied this first into the paired variables box) \> grade_test1 ('Measure 2' in jamovi). You should get the results shown in @fig-fig11-19.

```{r}
#| label: fig-fig11-19
#| classes: .enlarge-image
#| fig-cap: jamovi results showing a 'Paired Samples T-Test' where the actual hypothesis is one-sided, i.e. that grade_test2 ('Measure 1') $>$ grade_test1 ('Measure 2')
...
and he suggested a rough guide[^11-comparing-two-means-17a] for interpreting $d$ in @tbl-tab11-3.

```{r}
#| label: tbl-tab11-3
#| tbl-cap: A (very) rough guide to interpreting Cohen's $d$ 

mykbl(lsj_chapter_tables[[11]][[3]]) 

```

[^11-comparing-two-means-17a]: My personal recommendation is to not use these blindly. The $d$ statistic has a natural interpretation in and of itself. It re-describes the difference in means as the number of standard deviations that separates those means. So it is generally a good idea to think about what that means in practical terms. In some contexts a "small" effect could be of big practical importance. In other situations a "large" effect may not be all that interesting.

You'd think that this would be pretty unambiguous, but it's not. This is largely because Cohen wasn't too specific on what he thought should be used as the measure of the standard deviation (in his defence he was trying to make a broader point in his book, not nitpick about tiny details). 
...
p1 <- ggplot(df, aes(x=y)) +
  ...
  xlab("\nValue") +
  ylab("Frequency\n") +
  ggtitle("(a) Normally Distributed Data") +
  theme(
    plot.title = element_text(hjust = 0.5))
...
```{r out.width="80%"}
#| label: fig-fig11-22
#| fig-cap: jamovi QQ plot for the independent $t$-test analysis shown in @fig-fig11-10
...
```{r out.width="80%"}
#| label: fig-fig11-23
#| fig-cap: jamovi QQ plot for the paired samples $t$-test analysis shown in @fig-fig11-16
...
In the meantime, it's probably worth showing you an example of what happens to the QQ plot and the Shapiro-Wilk test when the data turn out to be non-normal. For that, let's look at the distribution of our AFL winning margins data, which if you remember back to @sec-Descriptive-statistics it didn't look like they came from a normal distribution at all. @fig-fig11-24 shows what happens to the QQ plot. 
And when we run the Shapiro-Wilk test on the *afl.margins* data, we get a value for the Shapiro-Wilk normality test statistic of $W = 0.94$, and $p-value = 9.481x10^{-07}$. Clearly a significant effect!

```{r out.width="80%"}
#| label: fig-fig11-24
#| fig-cap: jamovi QQ plot showing non-normality from the AFL winning margins data
...
knitr::include_graphics("images/fig11-24.png")
```
...
mykbl(lsj_chapter_tables[[11]][[4]], NULL) |>
  kableExtra::add_header_above(c(" " = 2, "Group B" = 5)) |>
  kableExtra::group_rows("Group A", 1, 5, label_row_css = "text-align: left;")  |>
...
In jamovi, if we run an 'Independent Samples T-Test' with scores as the dependent variable. and group as the grouping variable, and then under the options for 'tests' check the option for 'Mann-Whitney U', we will get results showing that $U = 3$ (i.e., the same number of checkmarks as shown above), and a $p$-value = $0.05556$. See @fig-fig11-25. Counting up the tick marks this time we get a test statistic of $W = 7$. As before, if our test is two-sided, then we reject the null hypothesis when W is very large or very small. 
...
mykbl(lsj_chapter_tables[[11]][[5]], NULL) |>
  kableExtra::add_header_above(c(" " = 2, "All differences" = 11)) |>
  kableExtra::group_rows("Positive\ndifferences", 1, 4, label_row_css = "text-align: left;") |>
   kableExtra::column_spec(3, border_right = TRUE) |>
   kableExtra::column_spec(1, width = "3em")

```

Running this in jamovi is straightforward. For the one sample version, specify the 'Wilcoxon rank' option under 'Tests' in the 'One Sample T-Test' analysis window. This gives a Wilcoxon $W = 7$, $p$-value = $0.03711$. Evidently, taking a statistics class does have an effect on your happiness. Switching to a paired samples version of the test won't give a different answer, of course; see @fig-fig11-26.
...
- [The paired-samples $t$-test] is used when you have two scores from each person, and you want to test the null hypothesis that the two scores have the same mean.
- [One-sided tests] are legitimate as long as they are pre-planned (like all tests!).
