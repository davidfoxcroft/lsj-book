...
knitr::include_graphics("images/fig11-1.png")
```
...
knitr::include_graphics("images/fig11-2.png")
```
...
knitr::include_graphics("images/fig11-3.png")
```
...
knitr::include_graphics("images/fig11-4.png")
```
...
knitr::include_graphics("images/fig11-5.png")
```
...
To give you a more detailed sense of what's going on here, I've plotted histograms (not in jamovi, but using R) showing the distribution of grades for both tutors (@fig-fig11-7), as well as a simpler plot showing the means and corresponding confidence intervals for both groups of students (@fig-fig11-8).
...
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: Histograms showing the distribution of grades for students in Anastasia's (panel a) and in Bernadette's (panel b) classes. Visually, these suggest that students in Anastasia's class may be getting slightly better grades on average, though they also seem a bit more variable
...
Not surprisingly, you can run an independent samples t-test easily in jamovi. The outcome variable for our test is the student grade, and the groups are defined in terms of the tutor for each class. So you probably won't be too surprised that all you have to do in jamovi is go to the relevant analysis ('Analyses' - 'T-Tests' - 'Independent Samples T-Test') and move the grade variable across to the 'Dependent Variables' box, and the tutor variable across into the 'Grouping Variable' box, as shown in @fig-fig11-10.
...
knitr::include_graphics("images/fig11-10.png")
```
...
If you tick the check box for the Welch test in the analysis we did above, then this is what it gives you (@fig-fig11-12).

```{r}
#| label: fig-fig11-12
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: Results showing the Welch test alongside the default Student's t-test in jamovi
...
If we take a quick look at the descriptive statistics, in @fig-fig11-13, we see that this impression seems to be supported. Across all 20 students the mean grade for the first test is 57%, but this rises to 58% for the second test. Although, given that the standard deviations are 6.6% and 6.4% respectively, it's starting to feel like maybe the improvement is just illusory; maybe just random variation. This impression is reinforced when you see the means and confidence intervals plotted in @fig-fig11-14 (a). If we were to rely on this plot alone, looking at how wide those confidence intervals are, we'd be tempted to think that the apparent improvement in student performance is pure chance.
...
knitr::include_graphics("images/fig11-14.png")
```

Once we have computed this new improvement variable we can draw a histogram showing the distribution of these improvement scores, shown in @fig-fig11-14 (c). When we look at the histogram, it's very clear that there is a real improvement here. The vast majority of the students scored higher on test 2 than on test 1, reflected in the fact that almost the entire histogram is above zero.
...
The output shown in @fig-fig11-15 is (obviously) formatted exactly the same was as it was the last time we used [The one-sample t-Test] analysis, and it confirms our intuition. There's an average improvement of $1.4\%$ from test 1 to test 2, and this is significantly different from $0$ $(t(19) = 6.48, p< .001)$.
...
#| fig-align: left
#| fig-width: 0.9
#| fig-cap: Histogram (panel (a)) and normal QQ plot (panel (b)) of normal.data, a normally distributed sample with $100$ observations. The Shapiro-Wilk statistic associated with these data is $W = .99$, indicating that no significant departures from normality were detected ($p = .73)$
knitr::include_graphics("images/fig11-20.png")
```
