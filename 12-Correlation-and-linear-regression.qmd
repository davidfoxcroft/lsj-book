...
![](images/fig12-1a.png){#fig-fig12-1a fig-align="left" width="30%"}

![](images/fig12-1b.png){#fig-fig12-1b fig-align="left" width="30%"}

![](images/fig12-1c.png){#fig-fig12-1c fig-align="left" width="30%"}
...
![](images/fig12-2a.png){#fig-fig12-2a fig-align="left" width="45%"}

![](images/fig12-2b.png){#fig-fig12-2b fig-align="left" width="45%"}
...
![](images/fig12-2a.png){#fig-fig12-3a fig-align="left" width="45%"}

![](images/fig12-3b.png){#fig-fig12-3b fig-align="left" width="45%"}
...
onePlot <- function( r,n=100 ) {
	...
	X <- MASS::mvrnorm(n,c(0,0),sigma, empirical=TRUE)
		
	ggplot(data.frame(X), aes(x=X[,1], y=X[,2])) +
	  geom_point(col="#3d6da9", size=.5) +
	  ylab(r) + 
	  theme_classic() +
	  theme(
	    ...
	    plot.background = element_rect(color = "lightgrey", size = 0.4))
}
...
theme_border <- theme_gray() + 
  theme(plot.background = element_rect(fill = NA, colour = 'black', size = 4))
...
```{r}
#| label: tbl-tab12-2
#| tbl-cap: A rough guide to interpreting correlations. Note that I say a rough guide. There aren't hard and fast rules for what counts as strong or weak relationships. It depends on the context.
huxtabs[[12]][[2]] 
```
...
The second way in which an observation can be unusual is if it has high **leverage**, which happens when the observation is very different from all the other observations. This doesn't necessarily have to correspond to a large residual. If the observation happens to be unusual on all variables in precisely the same way, it can actually lie very close to the regression line. An example of this is shown in @fig-fig12-18. The leverage of an observation is operationalised in terms of its hat value, usually written $h_i$ . The formula for the hat value is rather complicated[^12-correlation-and-linear-regression-18] but its interpretation is not: $h_i$ is a measure of the extent to which the i-th observation is "in control" of where the regression line ends up going.
...
In jamovi, information about Cook's distance can be calculated by clicking on the 'Cook's Distance' checkbox in the 'Assumption Checks' - 'Data Summary' options. When you do this, for the multiple regression model we have been using as an example in this chapter, you get the results as shown in @fig-fig12-20.
...
To give a sense of how we could end up with a model that has bigger collinearity problems, suppose I were to run a much less interesting regression model, in which I tried to predict the day on which the data were collected, as a function of all the other variables in the data set. To see why this would be a bit of a problem, let's have a look at the correlation matrix for all four variables (@tbl-tab12-5).

```{r}
#| label: tbl-tab12-5
#| tbl-cap: Correlation matrix for all four variables
huxtabs[[12]][[5]]
```
