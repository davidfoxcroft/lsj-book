...
huxtabs[[12]][[2]] |>
  set_width("60%") |>
  add_footnote(text = "*Note that I say a rough guide. There aren't hard and fast rules for what counts as strong or weak relationships. It depends on the context", border = 0.8) |>
  set_number_format(value = '%5.2f') |>
...
![](images/fig12-6a.png){width="45%"}
![](images/fig12-6b.png){width="45%"}
![](images/fig12-6c.png){width="45%"}
![](images/fig12-6d.png){width="45%"}
...
load("data_and_table_files/parenthood.Rdata")

p1 <- ggplot(parenthood, aes(dan.sleep, dan.grump)) +
  geom_point(col="darkgrey") +
  geom_smooth(method='lm', se = F, col=blueshade, size=1) +
...
p2 <- ggplot(parenthood, aes(dan.sleep, dan.grump)) +
  geom_point(col="darkgrey") +
  geom_abline(intercept = 80, slope = -3, col=blueshade, size=1) +
...
mydat <- parenthood |> tidyr::gather(key = "grp", value = "x", -1, -.data$predicted, -.data$residuals)
...
#| fig-align: left
#| out.width: 60%
#| fig-cap: jamovi plot of the predicted values against the observed values of the outcome variable. A straight(-ish) line is what we're hoping to see here. This looks pretty good, suggesting that there's nothing grossly wrong
...
Somewhat more advanced versions of the same plot is produced by checking 'Residuals plots' in the regression analysis 'Assumption checks' options in jamovi. These are useful not only for checking linearity, but also for checking normality and equality of variance assumptions, and we look at these in more detail in @Checking-the-normality-of-the-residuals. This option not only draws plots comparing the predicted values to the residuals, it does so for each individual predictor. 

### Checking the normality of the residuals {#Checking-the-normality-of-the-residuals}
...
![](images/fig12-20a.png){width="45%"}
![](images/fig12-20b.png){width="45%"}
![](images/fig12-20c.png){width="45%"}
![](images/fig12-20d.png){width="45%"}
...
ggplot(dat, aes(x=x, y=y)) +
  ...
  geom_smooth(method=lm, se=F, fullrange=T, col="black", size=0.8) +
  geom_smooth(data=dat2, method=lm, se=F, fullrange=T, linetype = 'dashed', col="black", size=0.4) +
  geom_segment(x=4.3, y=10.85, xend=4.3, yend=yendpoint, col="darkgrey", size = 0.4) + 
...
ggplot(dat, aes(x=x, y=y)) +
  geom_point(col=blueshade, size=3) +
  geom_smooth(method=lm, se=F, fullrange=T, col="black", size=0.8) +
  geom_smooth(data=dat2, fullrange=T, method=lm, se=F, linetype = 'dashed', col="black", size=0.4) +
  geom_segment(x=10, y=9.65, xend=10, yend=yendpoint, col="darkgrey", size = 0.4) + 
...
ggplot(dat, aes(x=x, y=y)) +
  ...
  geom_smooth(method=lm, se=F, fullrange=T, col="black", size=0.8) +
  geom_smooth(data=dat2, fullrange=T, method=lm, se=F, linetype = 'dashed', col="black", size=0.4) +
  geom_segment(x=10, y=4.1, xend=10, yend=yendpoint, col="darkgrey", size = 0.4) + 
