...
The logic behind a repeated measures ANOVA is very similar to that of an independent ANOVA (sometimes called a "between subjects" ANOVA). You'll remember that earlier we showed that in a between subjects ANOVA total variability is partitioned into between groups variability ($SS_b$) and within groups variability ($SS_w$), and after each is divided by the respective degrees of freedom to give $MS_b$ and $MS_w$ (see Table 13.1) the $F$-ratio is calculated as:
$$F=\frac{MS_b}{MS_w}$$
