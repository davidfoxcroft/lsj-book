...
```{r fig15-1, fig.cap=" Latent factor underlying the relationship between several observed variables", out.width = "90%"}
...
```{r fig15-2, fig.cap="Twenty-five observed variable items organised by five putative personality factors in the dataset bfi\\_sample.csv", out.width = "90%"}
...
```{r fig15-3, class.chunk = "enlarge-image", fig.cap="The jamovi EFA analysis window", out.width = "90%"}
knitr::include_graphics("images/Figure174.PNG")
```

```{r fig15-4, class.chunk = "enlarge-image", fig.cap="jamovi EFA assumption checks for the personality questionnaire data", out.width = "50%"}
...
```{r fig15-5, class.chunk = "enlarge-image", fig.cap="Scree plot of the personality data in jamovi EFA, showing a noticeable inflection and levelling off after point 5 (the 'elbow')", out.width = "90%"}
...
```{r fig15-6, class.chunk = "enlarge-image", fig.cap="Factor summary statistics and correlations for a five factor solution in jamovi EFA", out.width = "90%"}
...
```{r fig15-7, class.chunk = "enlarge-image", fig.cap="Factor loadings for a five factor solution in jamovi EFA", out.width = "90%"}
...
```{r fig15-8, class.chunk = "enlarge-image", fig.cap="Recode variable using the jamovi Transform command", out.width = "90%"}
knitr::include_graphics("images/Figure178.PNG")
```

```{r fig15-9, class.chunk = "enlarge-image", fig.cap="Compute new scale score variable using the jamovi Computed variable command", out.width = "90%"}
...
-   Use the Rj editor to run the EFA in R to the same specification as the one in jamovi (i.e. five factors and oblimin rotation) and compute optimally weighted factor scores (Figure \@ref(fig:fig15-10)). Save the new dataset, with the factor scores, to a file (Figure \@ref(fig:fig15-11)).

```{r fig15-10, class.chunk = "enlarge-image", fig.cap="Rj editor commands for creating optimally weighted factor scores for the five factor solution", out.width = "90%"}
knitr::include_graphics("images/Figure180.PNG")
```

```{r fig15-11, class.chunk = "enlarge-image", fig.cap="The newly created data file 'bfifactscores.csv'created in the Rj editor and containing the five factor score variables. Note that each of the new factor score variables is labelled corresponding to the order that the factors are listed in the factor loadings table", out.width = "90%"}
...
```{r fig15-12, class.chunk = "enlarge-image", fig.cap="Comparing differences in Agreeableness factor-based scores between males and females", out.width = "90%"}
...
Once you have loaded up the bfi_sample.csv data, select Factor - Principal Component Analysis from the main jamovi button bar to open the PCA analysis window (Figure \@ref(fig:fig15-13)). Then select the 25 personality questions and transfer them into the 'Variables' box. Check appropriate options, including 'Assumption Checks', but also Rotation 'Method', 'Number of Factors to extract, and 'Additional Output' options. See Figure \@ref(fig:fig15-13) for suggested options for this PCA, and please note that the Rotation 'Method' and 'Number of Factors' extracted is typically adjusted during the analysis to find the best result, as described below.

```{r fig15-13, class.chunk = "enlarge-image", fig.cap="The jamovi PCA analysis window", out.width = "90%"}
knitr::include_graphics("images/Figure183.PNG")
```

First, checking the assumptions (Figure \@ref(fig:fig15-14)). You can see that (1) Bartlett's test of sphericity is significant, so this assumption is satisfied; and (2) the KMO measure of sampling adequacy (MSA) is $0.81$ overall, suggesting very good sampling adequacy. No problems here then!

```{r fig15-14, class.chunk = "enlarge-image", fig.cap="jamovi PCA assumption checks for the personality item data", out.width = "80%"}
...
```{r fig15-15, class.chunk = "enlarge-image", fig.cap="Scree plot of the personality item data in jamovi PCA, showing the levelling off point, the 'elbow', after component 5", out.width = "90%"}
...
```{r fig15-16, class.chunk = "enlarge-image", fig.cap="Component summary statistics and correlations for a five component solution in jamovi PCA", out.width = "90%"}
...
```{r fig15-17, class.chunk = "enlarge-image", fig.cap="Component loadings for a five component solution in jamovi PCA", out.width = "90%"}
knitr::include_graphics("images/Figure187.PNG")
```

For components $1, 2, 3$ and $4$ the pattern of component loadings closely matches the putative factors specified in Figure \@ref(fig:fig15-2. And component $5$ is pretty close, with four of the five observed variables that putatively measure "openness" loading pretty well onto the component. Variable 04 doesn't quite seem to fit though, as the component solution in Figure \@ref(fig:fig15-17) suggests that it loads onto component $4$ (albeit with a relatively low loading) but not substantively onto component $5$.
...
```{r fig15-18, class.chunk = "enlarge-image", fig.cap="Rj editor commands for creating optimally weighted component scores for the five component solution", out.width = "90%"}
...
```{r fig15-19, class.chunk = "enlarge-image", fig.cap="Initial pre-specification of latent factor structure for the five factor personality scales, for use in CFA", out.width = "80%"}
...
```{r fig15-20, class.chunk = "enlarge-image", fig.cap="The jamovi CFA analysis window", out.width = "90%"}
...
```{r fig15-21, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Model Fit results for our CFA model", out.width = "70%"}
...
```{r fig15-22, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Factor Loadings table for our CFA model", out.width = "90%"}
knitr::include_graphics("images/Figure192.PNG")
```

```{r fig15-23, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Factor Covariances table for our CFA model", out.width = "90%"}
knitr::include_graphics("images/Figure193.PNG")
```

How could we improve the model? One option is to go back a few stages and think again about the items / measures we are using and how they might be improved or changed. Another option is to make some post hoc tweaks to the model to improve the fit. One way of doing this is to use "modification indices" (Figure \@ref(fig:fig15-24)), specified as an 'Additional output' option in jamovi.

```{r fig15-24, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Factor Loadings Modification Indices", out.width = "90%"}
...
```{r fig15-25, class.chunk = "enlarge-image", fig.cap="Residual Covariance Modification Indices produced by jamovi", out.width = "70%"}
knitr::include_graphics("images/Figure194a.PNG")
```
...
```{r fig15-26, class.chunk = "enlarge-image", fig.cap="The Attributional Style Questionnaire (ASQ) for negative events", out.width = "90%"}
...
```{r fig15-27, class.chunk = "enlarge-image", fig.cap="Six questions on the ASQ for each of the Internality, Stability and Globality dimensions", out.width = "80%"}
...
```{r fig15-28, class.chunk = "enlarge-image", fig.cap="Initial pre-specification of latent factor structure for the ASQ", out.width = "80%"}
...
```{r fig15-30, class.chunk = "enlarge-image", fig.cap="The jamovi CFA analysis window", out.width = "90%"}
knitr::include_graphics("images/Figure199.PNG")
```

Once we have set up the analysis we can turn our attention to the jamovi results window and see what's what. The first thing to look at is "Model fit" as this tells us how good a fit our model is to the observed data (Figure \@ref(fig:fig15-31)). NB in our model only the pre-specified covariances are estimated, everything else is set to zero, so model fit is testing both whether the pre-specified "free" parameters are not zero, and conversely whether the other relationships in the data -- the ones we have not specified in the model -- can be held at zero.

```{r fig15-31, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Model Fit results for our CFA MTMM model", out.width = "90%"}
...
```{r fig15-32, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Factor Loadings and Covariances tables for our CFA MTMM model", out.width = "90%"}
...
```{r fig15-33, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Factor Loadings Modification Indices", out.width = "80%"}
...
```{r fig15-34, class.chunk = "enlarge-image", fig.cap="The jamovi CFA Residual Covariances Modification Indices", out.width = "80%"}
...
-   Select Factor - Reliability Analysis from the main jamovi button bar to open the reliability analysis window (Figure \@ref(fig:fig15-35)).
...
```{r fig15-35, class.chunk = "enlarge-image", fig.cap="The jamovi Reliability Analysis window", out.width = "90%"}
...
```{r fig15-36, class.chunk = "enlarge-image", fig.cap="The jamovi Reliability Analysis results for the Agreeableness factor", out.width = "90%"}
