...
So how bad is it? The answer is shown as the solid line in @fig-fig16-1, and it's astoundingly bad. If you peek at your data after every single observation, there is a 37% chance that you will make a Type I error. That's, um, quite a bit bigger than the 5% that it's supposed to be. And it doesn't improve much with less frequent peeking: if you only peek every 10, or every 50 observations. then the Type I error rates are still way too high - 33% and 31% respectively. By way of comparison, imagine that you had used the following strategy. Start collecting data. Every single time an observation arrives, run [Bayesian t-tests] and look at the Bayes factor. I'll assume that @Johnson2013 is right, and I'll treat a Bayes factor of 3:1 as roughly equivalent to a p-value of .05.[^bayesian-statistics-14] This time around, our trigger happy researcher uses the following procedure. If the Bayes factor is 3:1 or more in favour of the null, stop the experiment and retain the null. If it is 3:1 or more in favour of the alternative, stop the experiment and reject the null. Otherwise continue testing. Now, just like last time, let's assume that the null hypothesis is true. What happens? As it happens, I ran the simulations for this scenario too, and the results are shown as the dashed line in @fig-fig16-1. It turns out that the Type I error rate for peeking every time a new observation arrives is 10% - much much lower than the 37% rate that we were getting by using the orthodox t-test. And for peeking every 10 or 50 observations the rates are 10% and 8%, respectively.

```{r}
#| label: fig-fig16-1
#| fig-cap: Probability of Type I error in an experiment with target N=1000 per group and "peeking" at different intervals:- things can go badly wrong if you "peek" at your data and re-run your tests as new data arrives. If you are a frequentist, this is *very wrong* (blue circles and solid line). If you are a bayesian it's better (green triangles and dashed line). The Type I error threshold is set at 0.05 (red dotted line)

## takes a long time to run so load saved data instead
#include"peeking_type_I_error.R"

p_bf <- readRDS("data_and_tables/p_bf.rds")

ggplot(p_bf) +
  geom_point(aes(x=peek, y=p_peek/1000), shape=1, col=blueshade, size = 2) +
  geom_line(aes(x=peek, y=p_peek/1000), stat="smooth", method = "loess",
    colour=blueshade, alpha=0.6, linetype = 'solid') +
  geom_point(aes(x=peek, y=bf_peek/1000), shape=2, col="#009E73", size = 2) +
  geom_line(aes(x=peek, y=bf_peek/1000), stat="smooth", method = "loess",
    colour="#009E73", alpha=0.6, linetype = 'dashed') +
  ...
  annotate(geom="text",label="alpha = .05", col = "red", size=2.5, x=3, y=0.035) +
  annotate(geom="text",label=" p < .05", x=90, y=0.35) +
  annotate(geom="text",label="BF > 3", x=90, y=0.13)
