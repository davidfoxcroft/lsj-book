...
```{r}
#| label: tbl-tab16-1
#| tbl-cap: How likely is it to rain in Adelaide - pre-existing beliefs based on knowledge average January rainfall
huxtabs[[16]][[1]]  
```
...
#| fig-align: left
#| out.width: 70%
#| fig-cap: How badly can things go wrong if you re-run your tests every time new data arrive? If you are a frequentist, the answer is *very wrong*
...
computeP <- function(x,g){
  reject_t <- t.test(formula=x~g)$p.value <= alpha
}
computeBF <- function(x,g){
  bayesttest <- BayesFactor::ttestBF( formula= x ~ g, data=data.frame(x,g))
  reject_BF <-  BayesFactor::extractBF(bayesttest)["bf"][[1]] >= bayes_crit
}
...
# takes a long time to run....beware!

# P_BF <- mclapply(1:N, function (n) {
#   if(n%%10==0) cat(".")
#   if(n%%200==0) cat(n, "\n")
#   for(m in 1:M) {
#     if(check(m,n)){
...
P <- readRDS("data_and_tabel_files\P.rds")
BF <- readRDS("data_and_tabel_files\BF.rds")
sumx <- function(x) {
 sum(x <= .05, na.rm=TRUE)
}
...
peeking_method <- function (observations, by=by){
  ...
  c(t=reject_t, BF=reject_BF)

}

calc_p_bf <- function(from, N, by, peek) {
  mclapply(seq(from, N, by), function(obs) {
    monte_carlo(1000, callback=peeking_method,
                           observations=obs,
                           by=peek)
...
### run the sim
# x - first data collected
# N - overall sample size
# testevery - number of data points collected after which you run a test
# 
...
library(parallel)
p_bf <- calc_p_bf(from=from,N=N,by=by,peek=peek)
saveRDS(p_bf, "data_and_table_files/p_bf.rds")
p_bf <- readRDS("data_and_table_files/p_bf.rds")

pdat <- lapply(p_bf, head, 1)
pdat <- matrix(unlist(pdat), ncol=1000, byrow=TRUE)
p <- apply(pdat,1,sumT) / 1000
p

bfdat <- lapply(p_bf, tail, 1)
bfdat <- matrix(unlist(bfdat), ncol=1000, byrow=TRUE)
bf <- apply(bfdat,1,sumT) / 1000
bf
...
plotdat <- data.frame(Ssize=seq(1, N, by), p=p, bf=bf)


ggplot(plotdat) +
  geom_smooth(aes(x=Ssize, y=p), se=F) +
  geom_smooth(aes(x=Ssize, y=bf), se=F) 
