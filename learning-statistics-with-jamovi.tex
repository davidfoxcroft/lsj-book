...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, coltitle=black, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, colback=white, rightrule=.15mm, toprule=.15mm, arc=.35mm, left=2mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, bottomtitle=1mm, opacityback=0, titlerule=0mm, bottomrule=.15mm, leftrule=.75mm, breakable, opacitybacktitle=0.6]
...
\chapter{Categorical data analysis}\label{sec-Categorical-data-analysis}
...
pronounced ``chi-square''\footnote{Also sometimes referred to as
  ``chi-squared''.} and \(t\)-tests in
\textbf{?@sec-Comparing-two-means}). Both of these tools are very
...
Fisher (Fisher, 1922). It tests whether an observed frequency
...
With this in mind, let's consider a very simple study. Suppose I asked
people to imagine a shuffled deck of cards, and mentally pick one card
from this imaginary deck ``at random''. After they've chosen one card I
ask them to mentally select a second one. For both choices what we're
going to look at is the suit (hearts, clubs, spades or diamonds) that
people chose. After asking, say, \(N = 200\) people to do this, I'd like
to look at the data and figure out whether or not the cards that people
pretended to select were really random. The data are contained in the
\emph{randomness.csv} file in which, when you open it up in jamovi and
take a look at the spreadsheet view, you will see three variables. These
are: an id variable that assigns a unique identifier to each
participant, and the two variables choice\_1 and choice\_2 that indicate
the card suits that people chose.
...
\caption{\label{tbl-tab10-1}Number of times each suit was chosen}
...
\caption{\label{tbl-tab10-2}Relationship between English descriptions
...
vector,\footnote{A vector is a sequence of data elements of the same
  basic type: \[O = (O_1, O_2, O_3, O_4)\] Again, this is nothing new or
  interesting. It's just notation. If I say that
  \(O = (35, 51, 64, 50)\) all I'm doing is describing the table of
  observed frequencies (i.e., observed), but I'm referring to it using
  mathematical notation.} which I'll refer to as \(O\):
...
\[H_0: \text{ All four suits are chosen with equal probability}\] Now,
let's say this in a mathematical way. To do this, let's use the notation
\(P_j\) to refer to the true probability that the j-th suit is chosen.
If the null hypothesis is true, then each of the four suits has a 25\%
chance of being selected. In other words, our null hypothesis claims
that \(P_1 = .25\), \(P_2 = .25\), \(P_3 = .25\) and finally that
\(P_4 = .25\) .

However, in the same way that we can group our observed frequencies into
a vector O that summarises the entire data set, we can use P to refer to
the probabilities that correspond to our null hypothesis. So if I let
the vector \(P = (P_1, P_2, P_3, P_4)\) refer to the collection of
probabilities that describe our null hypothesis, then we have:
\[H_0: P =(.25, .25, .25, .25)\]

In this instance, our null hypothesis corresponds to a vector of
probabilities \(P\) in which all of the probabilities are equal to one
another. But this doesn't have to be the case: if the experimental task
was for people to imagine they were drawing from a deck that had twice
as many clubs as any other suit, then the null hypothesis would be
\(P = (.4, .2, .2, .2)\).
...
However, the typical use of the goodness-of-fit test is with a null
hypothesis that all the categories are equally likely, so we'll stick to
...
expecting if the null is true'', then: \[E_i=N \times P_i\]
...
\caption{\label{tbl-tab10-3}Expected and observed frequencies}
...
\caption{\label{tbl-tab10-4}Squaring the difference scores}
...
\caption{\label{tbl-tab10-5}Dividing the squared difference scores by
the expected frequency to provide an ``error'' score}
...
\textbf{?@sec-The-binomial-distribution} in
\textbf{?@sec-Introduction-to-probability}. In other words, if the null
...
\textbf{?@sec-The-central-limit-theorem} the binomial distribution
...
\textbf{?@sec-Other-useful-distributions}, when you take a bunch of
...
you remember back to \textbf{?@sec-Other-useful-distributions}, I said
that if the number of things you're adding up is \(k\), then the degrees
of freedom for the resulting chi-square distribution is \(k\). Yet, what
I said at the start of this section is that the actual degrees of
...
\textbf{?@sec-Other-useful-distributions}, I was a bit vague about what
\textbf{``degrees of freedom''} actually means. Obviously, it matters.
Looking at Figure~\ref{fig-fig10-1}, you can see that if we change the
degrees of freedom then the chi-square distribution changes shape quite
substantially. But what exactly is it? Again, when I introduced the
distribution and explained its relationship to the normal distribution,
I did offer an answer: it's the number of ``normally distributed
variables'' that I'm squaring and adding together. But, for most people,
that's kind of abstract and not entirely helpful. What we really need to
do is try to understand degrees of freedom in terms of our data. So here
goes.

\begin{figure}[h!]

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{10-Categorical-data-analysis_files/figure-pdf/fig-fig10-1-1.pdf}
...
\textbf{?@sec-Hypothesis-testing} the chi-square goodness-of-fit test is
always a \textbf{one-sided test}. Right, so all we have to do is figure
out what this critical value is. And it's pretty straightforward. If we
want our test to have significance level of \(\alpha = .05\) (that is,
we are willing to tolerate a type I error rate of \(5%
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{10-Categorical-data-analysis_files/figure-pdf/fig-fig10-2-1.pdf}
...
Table~\ref{tbl-tab10-6}. Looking at this Figure, we can see that the
critical value for a \(\chi^2\) distribution with 3 degrees of freedom
and \(p=0.05\) is 7.815.

\begin{table}

\caption{\label{tbl-tab10-6}Table of critical values for the chi-square
distribution}
...
Tests' -- `\(N\) Outcomes'. Then in the analysis window that appears
move the variable you want to analyse (choice 1 across into the
`Variable' box. Also, click on the `Expected counts' check box so that
these are shown on the results table. When you have done all this, you
should see the analysis results in jamovi as in
Figure~\ref{fig-fig10-3}. No surprise then that jamovi provides the same
expected counts and statistics that we calculated by hand above, with a
\(\chi^2\) value of \((8.44\) with \(3\) df and \(p=0.038\). Note that
we don't need to look up a critical \(p\)-value threshold value any
more, as jamovi gives us the actual \(p\)-value of the calculated
\(\chi^2\) for 3 \(df\).

\begin{figure}[h!]

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig10-3.png}

\caption{\label{fig-fig10-3}A \(\chi^2\) One Sample Proportion Test in
...
\caption{\label{tbl-tab10-7}Expected counts for a different null
hypothesis}
...
This seems like a silly theory to me, and it's pretty easy to test this
explicitly specified null hypothesis with the data in our jamovi
analysis. In the analysis window (labelled `Proportion Test (N
Outcomes)' in Figure~\ref{fig-fig10-3} you can expand the options for
`Expected Proportions'. When you do this, there are options for entering
different ratio values for the variable you have selected, in our case
this is choice 1. Change the ratio to reflect the new null hypothesis,
as in Figure~\ref{fig-fig10-4}, and check the results.

\begin{figure}[h!]

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{images/fig10-4.png}

\caption{\label{fig-fig10-4}Changing the expected proportions in the
...
\(\chi^2\) test statistic is different, and our \(p\)-value is different
too. Annoyingly, the \(p\)-value is \(.192\), so we can't reject the
null hypothesis (look back at \textbf{?@sec-The-p-value-of-a-test} to
...
\textbf{?@sec-Tabulating-and-cross-tabulating-data}). In jamovi we do
this using the `Frequencies' -- `Contingency Tables' -- `Independent
Samples' analysis, and we should get a table something like
Table~\ref{tbl-tab10-8}.

\begin{table}

\caption{\label{tbl-tab10-8}Cross-tabulating the data}
...
some notation to describe the data (Table~\ref{tbl-tab10-9}).

\begin{table}

\caption{\label{tbl-tab10-9}Notation to describe the data}
...
choice, we can let Pi refer to this probability, e.g., \(P_1\) is the
true probability of choosing the puppy.
...
\(P_i\), then the expected frequency is just: \[E_{ij}=C_j \times P_i\]
...
doesn't actually specify a particular value for P\(i\).

It's something we have to estimate (see
\textbf{?@sec-Estimating-unknown-quantities-from-a-sample}) from the
...
estimate for the probability of choosing option \(i\) is just the row
total divided by the total sample size: \[\hat{P}_{i}= \frac{R_i}{N}\]
...
the total number of observations: \[{E}_{ij}= \frac{R_i \times C_j}{N}\]
{[}Additional technical detail\footnote{Technically, \(E_{ij}\) here is
  an estimate, so I should probably write it \(\hat{E_{ij}}\), but since
  no-one else does, I won't either. Now that we've figured out how to
  calculate the expected frequencies, it's straightforward to define a
  test statistic, following the exact same strategy that we used in the
  goodness-of-fit test. In fact, it's pretty much the same statistic.
  For a contingency table with r rows and c columns, the equation that
  defines our \(X^2\) statistic is:
...
constraints? Here, it's slightly trickier. The answer is always the
same: \[df=(r-1)(c-1)\]
...
Remember how our null hypothesis had some free parameters (i.e., we had
to estimate the Pi values)? Those matter too. I won't explain why in
this book, but every free parameter in the null hypothesis is rather
...
randomly sampled person turns out to be a robot.\footnote{A problem many
  of us worry about in real life.}

Finally, since we did actually fix the total number of observations N,
that's one more constraint. So, now we have \(rc\) observations, and
\((c-1)+(r-1)+1\) constraints. And that gives: \[
...
After you have run the test in jamovi (`Frequencies' -- `Contingency
Tables' -- `Independent Samples'), all you have to do is look underneath
the contingency table in the jamovi results window and there is the
\(\chi^2\) statistic for you. This shows a \(\chi^2\) statistic value of
10.72, with 2 df and \(p\)-value = 0.005.
...
Cramér's \(V\) of \(0.24\). See Figure~\ref{fig-fig10-5}. We will talk
about this some more in just a moment.

\begin{figure}[h!]

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig10-5.png}

\caption{\label{fig-fig10-5}Independent samples \(\chi^2\) test in
...
As far as I can tell from reading Yates' paper,\footnote{Yates (1934)
  suggested a simple fix, in which you redefine the goodness-of-fit
  statistic as: \[\chi^{2}=\sum_{i}\frac{(|E_i-O_i|-0.5)^2}{E_i}\]
  Basically, he just subtracts off 0.5 everywhere.} the correction is
...
it is called `\(\chi^2\) continuity correction'.

\section{Effect size}\label{effect-size}

As we discussed earlier in
\textbf{?@sec-Effect-size-sample-size-and-power}, it's becoming
...
\item
  ...
  \textbf{?@sec-Introduction-to-probability} this is only true when the
  number of observations is sufficiently large. What that means in
...
\caption{\label{tbl-tab10-10}Contingency table of the \emph{salem.csv}
data}
...
is where \textbf{Fisher's exact test} (Fisher, 1922) comes in very
handy.
...
for a \(2 \times 2\) contingency table. As before, let's have some
notation (Table~\ref{tbl-tab10-11}).

\begin{table}

\caption{\label{tbl-tab10-11}Notation for the Fisher exact test}
...
\textbf{?@sec-Introduction-to-probability} this is written:
...
probability of observing this particular table or a table that is
\emph{``more extreme''}.\footnote{Not surprisingly, the Fisher exact
  test is motivated by Fisher's interpretation of a \(p\)-value, not
...
people not on fire. See Figure~\ref{fig-fig10-6}.

\begin{figure}[h!]

\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{images/fig10-6.png}

\caption{\label{fig-fig10-6}Fisher exact test analysis in jamovi}
...
the contingency table shown in Table~\ref{tbl-tab10-12}.

\begin{table}

\caption{\label{tbl-tab10-12}Contingency table with data on AGPP
...
\hyperref[the-chi2-test-of-independence-or-association]{The \(\chi^2\)
test of independence (or association)}). However, a little bit of
thought reveals that we've got a problem. We have 100 participants but
200 observations. This is because each person has provided us with an
answer in both the before column and the after column. What this means
is that the 200 observations aren't independent of each other. If voter
A says ``yes'' the first time and voter B says ``no'', then you'd expect
...
is a standard repeated measures design, and none of the tests we've
...
\caption{\label{tbl-tab10-13}Tabulate the data in a different way when
...
and this statistic has a \(\chi^2\) distribution (approximately) with
\(df = 1\). However, remember that just like the other \(\chi^2\) tests
it's only an approximation, so you need to have reasonably large
expected cell counts for it to work.
...
McNemar test just below it, see Figure~\ref{fig-fig10-7}.

\begin{figure}[h!]

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{images/fig10-7.png}

\caption{\label{fig-fig10-7}McNemar test output in jamovi}
...
(Table~\ref{tbl-tab10-14}).

\begin{table}

\caption{\label{tbl-tab10-14}Cross-tabulating first against second
choice with the \emph{Randomness.omv} (cards) data}
...
in Figure~\ref{fig-fig10-8}. Notice that the results are different!
These aren't the same test.

\begin{figure}[H]

\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{images/fig10-8.png}

\caption{\label{fig-fig10-8}Independent vs.~Paired (McNemar) with the
...
\tightlist
\item
  \hyperref[the-chi2-chi-square-goodness-of-fit-test]{The \(\chi^2\)
  (chi-square) goodness-of-fit test} is used when you have a table of
  observed frequencies of different categories, and the null hypothesis
  gives you a set of ``known'' probabilities to compare them to.
\item
  \hyperref[the-chi2-test-of-independence-or-association]{The \(\chi^2\)
  test of independence (or association)} is used when you have a
  contingency table (cross-tabulation) of two categorical variables. The
  null hypothesis is that there is no relationship or association
  between the variables.
\item
  \hyperref[effect-size]{Effect size} for a contingency table can be
  measured in several ways. In particular we noted the Cramér's \(V\)
  statistic.
\item
  ...
  are independent (\hyperref[assumptions-of-the-tests]{Assumptions of
  the test(s)}. \hyperref[the-fisher-exact-test]{The Fisher exact test}
  can be used when the expected frequencies are small.
  \hyperref[sec-The-McNemar-test]{The McNemar test} can be used for some
...
\bibitem[\citeproctext]{ref-Agresti1996}
Agresti, A. (1996). \emph{An introduction to categorical data analysis}.
Wiley. \url{https://doi.org/10.1002/0470114754}
...
\bibitem[\citeproctext]{ref-Cochran1954}
...
\bibitem[\citeproctext]{ref-Cramer1946}
Cramer, H. (1946). \emph{Mathematical methods of statistics}. Princeton
University Press. \url{https://doi.org/10.1515/9781400883868}

\bibitem[\citeproctext]{ref-Fisher1922a}
...
\url{https://doi.org/10.1111/j.2397-2335.1922.tb00768.x}

\bibitem[\citeproctext]{ref-Hogg2005}
Hogg, R. V., McKean, J. V., \& Craig, A. T. (2005). \emph{Introduction
to mathematical statistics} (6th ed.). Pearson.

\bibitem[\citeproctext]{ref-Larntz1978}
...
\url{https://doi.org/10.1080/01621459.1978.10481567}
