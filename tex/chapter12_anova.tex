...
$$
where the true value $Q=0$ if the null hypothesis is true, and $Q > 0$ if the alternative hypothesis is true \cite<e.g.>[ch.\ 10]{Hays1994}. Therefore, at a bare minimum {\it the $F$ value must be larger than 1} to have any chance of rejecting the null hypothesis. Note that this {\it doesn't} mean that it's impossible to get an $F$-value less than 1. What it means is that, if the null hypothesis is true the sampling distribution of the $F$ ratio has a mean of 1,\FOOTNOTE{Or, if we want to be sticklers for accuracy, $1 + \frac{2}{df_2 - 2}$.} and so we need to see $F$-values larger than 1 in order to safely reject the null.
...
$$
\noindent
Of course, if we actually wanted to get the {\it right} answer, we'd need to do this for all 18 observations in the data set, not just the first five. We could continue with the pencil and paper calculations if we wanted to, but it's pretty tedious. Alternatively, it's not too hard to get jamovi to do it (apart from the group means which you probably have to type in) but in fact this sort of thing is often better done in a dedicated spreadsheet programme such as OpenOffice or Excel, or in \R. However you do it, you should end up with a within-group sum of squares value of 1.39. 
...
\end{array}
$$
As you can see, the between group calculations are a lot shorter, so you probably wouldn't usually want to bother using jamovi or another programme as your calculator. 
...
There's more than one way to skin a cat, as the saying goes, and more than one way to test the homogeneity of variance assumption, too (though for some reason no-one made a saying out of that). The most commonly used test for this that I've seen in the literature is the \keyterm{Levene test} \cite{Levene1960} which is implemented in jamovi, and the closely related \keyterm{Brown-Forsythe test} \cite{BrownForsythe1974} which is not currently available in jamovi. Howver, I'll describe both of them here.
