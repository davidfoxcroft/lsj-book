...
Suppose my goal is to draw a scatterplot displaying the relationship between the amount of sleep that I get (\rtext{dan.sleep}) and how grumpy I am the next day (\rtext{dan.grump}). There are two different ways in which we can use jamovi to get the plot that we're after. The first way is to use the `Plot' option under the `Regression' - `Correlation Matrix' button, giving us the output shown in Figure \ref{fig:scatterplot1}. Note that jamovi draws a line through the points, we'll come onto this a bit later in Section~(\ref{sec:introregression}). Plotting a scatterplot in this way also allow you to specify `Densities for variables' and this option adds a density curve showing how the data in each variable is distributed. 
...
To run my linear regression, open up the `Regression' - `Linear Regression' analysis in jamovi, using the \rtext{parenthood.csv} data file. Then specify \rtext{dan.grump} as the `Dependent Variable' and \rtext{dan.sleep} as the variable entered in the `Covariates' box. This gives the results shown in Figure \ref{fig:reg1}, showing an intercept $\hat{b}_0 = 125.96$ and the slope $\hat{b}_1 = -8.94$. In other words, the best-fitting regression line that I plotted in Figure~\ref{fig:regression1} has this formula:
$$
...
\caption{A 3D visualisation of a multiple regression model. There are two predictors in the model, \rtext{dan.sleep} and \rtext{baby.sleep} and the outcome variable is \rtext{dan.grump}. Together, these three variables form a 3D space. Each observation (dot) is a point in this space. In much the same way that a simple linear regression model forms a line in 2D space, this multiple regression model forms a plane in 3D space. When we estimate the regression coefficients what we're trying to do is find a plane that is as close to all the blue dots as possible.}
...
\SUBSECTION{The $R^2$ value~\label{sec:rsquared}}
...
\noindent
1. create a new column called `\rtext{Y.pred}' using the formula `\rtext{= 125.97 + (-8.94 * dan.sleep)}'
...
\noindent
2. calculate the SS(resid) by creating a new column called '\rtext{(Y-Y.pred)\^{}2}' using the formula \\`\rtext{ = (dan.grump - Y.pred)\^{}2}'.
...
\noindent
4. At the bottom of the dan.grump column, calculate the mean value for \rtext{dan.grump} \\(NB Excel uses the word `\rtext{AVERAGE}' rather than `mean' in its function).

\noindent
5. Then create a new column, called `\rtext{(Y - mean(Y))\^{}2 )}' using the formula \\`\rtext{ = (dan.grump - AVERAGE(dan.grump))\^{}2 } '.
...
\SUBSECTION{The adjusted \boldm{$R^2$} value}
...
$$
for all values of $\lambda$ except $\lambda = 0$. When $\lambda = 0$ we just take the natural logarithm (i.e., $\ln(x)$). You can calculate it using the \rtext{BOXCOX} function in the `Compute' variables screen in jamovi. 
...
\begin{figure}[!htb]
\begin{center}
\epsfig{file = ../img/regression/reg8.png, clip=true,width = 13.5cm}
